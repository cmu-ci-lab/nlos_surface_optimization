...
def optimization(ratio):
	lr0 = 0.0001
	lr = lr0
	folder_name = os.getcwd() + '/progress-b-8-%f/'% ratio
	...
	for t in range(5000):
	  ...
	  global_counter, l2_record = optimize_parameters.optimize_alpha(mesh, gt_transient, weight, opt, 50, global_counter, folder_name)
	  if t == 0:
	    l2_0 = l2_record

	  lr = (l2_record/l2_0) * lr0 * ((0.99)**(resolution_cnt/2))
	  print('new lr %f' % lr)
