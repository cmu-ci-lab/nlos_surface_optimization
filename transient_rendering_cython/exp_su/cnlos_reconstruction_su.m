function [x, y, depth, albedo, vol]  = cnlos_reconstruction_su(scene, z_range)
...
switch scene
    ...
    case {7}
        load data_s_u.mat
        z_offset = 800;
        z_offset = max(z_offset, z_range(1));
    case {8}
...
vol = vol(:,:,end:-1:1);
vol = vol(1+z_offset:min([z_range(2), ind+z_offset,size(vol,1)]),:,:);
