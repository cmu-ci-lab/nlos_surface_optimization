	...
	for (int32_t iterSample = 0; iterSample < numSamples; ++iterSample) {
		...
		if ((rays[iterSample].geomID != RTC_INVALID_GEOMETRY_ID) && (rays[iterSample].primID == triangleIndex)) {
			...
			if ((halfLength <= pathlengthUpperBound / 2.0f)
				...
				float BRDF = eval(alpha, normal, -rays[iterSample].dir); 
                                intensity[triangleIndex] += (double) (faceArea * albedo * formFactor * formFactor * BRDF)
								/ (double) numSamples;
			}
		}
	}
		...
		if ((rays[iterSample].geomID != RTC_INVALID_GEOMETRY_ID) && (rays[iterSample].primID == triangleIndex)) {
			...
			if ((halfLength <= pathlengthUpperBound / 2.0f)
				...
				int32_t bin = (int32_t) embree::floor((2.0f * halfLength - pathlengthLowerBound)
														/ pathlengthResolution);
				float BRDF = eval(alpha, normal, -rays[iterSample].dir);
				transient[sourceIndex*numBins + bin] += (double) (faceArea * albedo * formFactor * formFactor * BRDF)
								/ (double) numSamples;
			}
		}
	}
	...
	parallel_for(int(0), int(numTriangles*numSources),[&](const range<int>& range) {
	 	...
	 	for (int iterTriangle_source = range.begin(); iterTriangle_source < range.end();															iterTriangle_source++) {
			streamedRayTraceIntensity(iterTriangle_source, g_device, g_scene, samplers[threadIndex],
				originD, normalD, verticesD, trianglesD, vertexNormal, alpha, 
	...
	parallel_for(int(0), int(numTriangles*numSources),[&](const range<int>& range) {
	 	...
	 	for (int iterTriangle_source = range.begin(); iterTriangle_source < range.end();															iterTriangle_source++) {
			streamedRayTraceTriangle(iterTriangle_source, g_device, g_scene, samplers[threadIndex],
				originD, normalD, verticesD, trianglesD, vertexNormal, vertexAlbedo, alpha,
...
void streamedRayTraceTriangleGradientAlpha(int32_t triangle_source_index,
					RTCDevice g_device,
					RTCScene g_scene,
					smp::Sampler &sampler,
					const float *originD,
		...
		if ((rays[iterSample].geomID != RTC_INVALID_GEOMETRY_ID) && (rays[iterSample].primID == triangleIndex)) {
			...
			if ((halfLength <= pathlengthUpperBound / 2.0f)
				...
				float formFactor = cos_theta2 * cos_theta3/ halfLength
							     / halfLength;
				float BRDF_da = eval_adiff(alpha, normal, -rays[iterSample].dir);
                                double g = albedo * formFactor *formFactor * BRDF_da;
					...
					/ pathlengthResolution);
                                ...
                                // compute gradient 
				    gradient[0] += (double) faceArea * g * weighting_kernal[i] * (-2)*difference[sourceIndex*numBins + bin] / (double) numSamples;
                                 }
...
double render_smoothed_gradients_alpha(int numBins, 
	...
	smp::SamplerSet samplers(numThreads);
        ...
        double *gradients = (double *) aligned_alloc(16, numThreads * sizeof(gradients));
        if (gradients == NULL) {
	...
	parallel_for(int(0), int(numTriangles*numSources),[&](const range<int>& range) {
	    	const int threadIndex = (int) TaskScheduler::threadIndex();
	 	for (int iterTriangle_source = range.begin(); iterTriangle_source < range.end();															iterTriangle_source++) {
			streamedRayTraceTriangleGradientAlpha(iterTriangle_source, g_device, g_scene, samplers[threadIndex],
				originD, normalD, verticesD, trianglesD, vertexNormal, vertexAlbedo, alpha,
		...
		if ((rays[iterSample].geomID != RTC_INVALID_GEOMETRY_ID) && (rays[iterSample].primID == triangleIndex)) {
			...
			if ((halfLength <= pathlengthUpperBound / 2.0f)
				...
				float formFactor = cos_theta2 * cos_theta3/ halfLength
							     / halfLength;
				float BRDF = eval(alpha, normal, -rays[iterSample].dir);
				Vec3fa BRDF_dn, BRDF_dw, BRDF_dx;
				eval_nwdiff(alpha, normal, -rays[iterSample].dir, BRDF_dn, BRDF_dw);
				BRDF_dx = -BRDF_dw + rays[iterSample].dir*dot(rays[iterSample].dir, BRDF_dw)/halfLength;
				
                                double intensity = albedo * formFactor *formFactor * BRDF;
                                
                                // compute gradient 
				Vec3fa t1_1 = 2 * cos_theta2 * cos_theta3 * (originNormal * cos_theta3 - normal * cos_theta2 + 4 * (-rays[iterSample].dir) * cos_theta2 * cos_theta3);
                                t1_1 /= embree::pow(halfLength,5);
				t1_1 *= BRDF;
				Vec3fa t1_2 = formFactor*formFactor*BRDF_dx;
				Vec3fa t1 = t1_1 + t1_2;
                                ...
                                if (testing_flag == 0 && normals != nullptr) {
                                    gn = - 2 * rays[iterSample].dir * cos_theta3 * cos_theta2 * cos_theta2 * BRDF;
                                    gn /= embree::pow(halfLength, 4);
				    gn += formFactor * formFactor * BRDF_dn;
