...
void render_smoothed_transients( int numBins, 
	...
	smp::SamplerSet samplers(numThreads);
        double *transient_tmp = (double*) aligned_alloc(16, numBins*numSources*sizeof(double));
        std::memset(transient_tmp, 0, numBins*numSources*sizeof(double));
	...
	double *transients = (double *) aligned_alloc(16, numThreads * numBins *sizeof(double) * numSources);
        ...
        std::memset(transients, 0, numThreads * numBins* sizeof(transients) *numSources);
  	...
  	parallel_for(int(0), int(numSources),[&](const range<int>& range) {
	    ...
	    for (int iterSource = range.begin(); iterSource < range.end(); iterSource++) {
		   ...
		   for (int iterBin = 0; iterBin < numBins; ++ iterBin) {
		     transient_tmp[iterSource*numBins + iterBin] += transients[(iterThread * numSources + iterSource)* numBins + iterBin];
 		   }
                }
	    }
	});

        double* convolution_kernal = (double*) aligned_alloc(16, numThreads*weight_length*sizeof(double));
        double* y = (double*) aligned_alloc(16, numThreads*(numBins + weight_length - 1)*sizeof(double));
        for (int i = 0; i < numThreads; ++i) {
            std::memcpy(&convolution_kernal[i*weight_length], weight, weight_length*sizeof(double));
        }
        ...
        parallel_for(int (0), int(numSources), [&](const range<int>& range) {
            ...
            for (int iterS = range.begin(); iterS < range.end(); ++iterS) {
                double* new_y = &y[threadIndex*(numBins+weight_length-1)];
                std::memset(new_y, 0, (numBins+weight_length-1)*sizeof(double));
                convolve(&convolution_kernal[threadIndex*weight_length], &transient_tmp[iterS*numBins], new_y, weight_length, numBins, numBins+weight_length-1, 0);
	
                for (int iterBin = 0; iterBin < numBins ; ++iterBin) {
                    transient[iterS*numBins+iterBin] += new_y[iterBin + weight_offset];
                }
            }
        });       
...
void render_smoothed_gradients(int numBins, 
	...
	smp::SamplerSet samplers(numThreads);
        ...
        std::memset(gradients, 0, numThreads * 3 * numVertices * sizeof(gradient));
        double* weighting_kernal = (double*) aligned_alloc(16, numThreads*jitter_length*sizeof(double));
        double* weighting_grad = (double*) aligned_alloc(16, numThreads*jitter_length*sizeof(double));
        for (int i = 0; i < numThreads; ++i) {
            std::memcpy(&weighting_kernal[i*(jitter_length)], jitter_weight, jitter_length*sizeof(double));
	    std::memcpy(&weighting_grad[i*jitter_length], jitter_grad, jitter_length*sizeof(double));
        } 
	parallel_for(int(0), int(numTriangles*numSources),[&](const range<int>& range) {
	    	const int threadIndex = (int) TaskScheduler::threadIndex();
	 	for (int iterTriangle_source = range.begin(); iterTriangle_source < range.end();															iterTriangle_source++) {
			streamedRayTraceTriangleGradient(iterTriangle_source, g_device, g_scene, samplers[threadIndex],
				...
				numSamplesPerTriangle, difference, &gradients[threadIndex*3*numVertices], 
                                numTriangles, numVertices, 
                                &weighting_kernal[threadIndex*jitter_length], &weighting_grad[threadIndex*jitter_length], jitter_offset, jitter_length, testing_flag);
                }
			...
			if ((halfLength <= pathlengthUpperBound / 2.0f)
				...
				Vec3fa t1 = 2 * albedo * cos_theta2 * cos_theta3 * (originNormal * cos_theta3 - normal * cos_theta2 + 4 * (-rays[iterSample].dir) * cos_theta2 * cos_theta3);
                                ...
                                for (int i = 0; i < jitter_length; ++i) {
                                    delta_bin = i - jitter_offset ;
				    bin = (int32_t) embree::floor((2.0f * halfLength - pathlengthLowerBound)
														/ pathlengthResolution) + delta_bin;
                                    e = v3 - v2;
                                    g = (t1 * jitter_weight[i] + jitter_grad[i] * intensity * (- 2) * rays[iterSample].dir/pathlengthResolution )*u + cross(t2,e) * jitter_weight[i];
				    g *= (-2)*difference[sourceIndex * numBins + bin];
                                    ...
                                    e = v1-v3;
                                    g = (t1 * jitter_weight[i] + jitter_grad[i] * intensity * (-2) * rays[iterSample].dir /pathlengthResolution )*v + cross(t2,e) * jitter_weight[i];
			   	    g *= (-2)*difference[sourceIndex * numBins + bin];
                                    ...
                                    e = v2-v1;
                                    g = (t1 * jitter_weight[i] + jitter_grad[i] * intensity * (-2) * rays[iterSample].dir /pathlengthResolution  )*w + cross(t2,e) * jitter_weight[i];
				    g *= (-2)*difference[sourceIndex * numBins + bin];
