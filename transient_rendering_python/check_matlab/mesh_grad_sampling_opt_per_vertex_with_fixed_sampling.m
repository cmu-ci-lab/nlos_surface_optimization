function grad = mesh_grad_sampling_opt_per_vertex_with_fixed_sampling(vertex, mesh, lighting, sensor, lighting_normal, sensor_normal, barycentric_coord,sampled_face, opt)
...
grad = zeros(1,3);
mesh_area = doublearea(mesh.v,mesh.f)/2;
...
for f = 1:length(related_face)
    ...
    grad = grad + sum(g(inds,:),1)*mesh_area(related_face(f))/sample_num;
    %grad = grad + sum(g(inds,:),1)*mesh_area(related_face(f));
    
%     u = unique(distance_bin(inds));
%     for j = 1:length(u)
%         grad(u(j),:) = grad(u(j),:) + sum(g(distance_bin == u(j),:),1);
%     end
    
end

%grad = grad/size(sampled_face,1);
