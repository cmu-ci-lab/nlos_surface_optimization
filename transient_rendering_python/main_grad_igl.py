...
tic = time.time()
angular_transient = rendering_grad_igl.angular_sampling(mesh, direction, triangleIndexMap, lighting, sensor, lighting_normal, sensor_normal, opt)
print(' igl forward')
print(time.time() - tic)
...
tic = time.time()
angular_transient_compare = rendering_grad.angular_sampling(mesh, direction, lighting, sensor, lighting_normal, sensor_normal, opt)
print(' forward')
print(time.time() - tic)

tic = time.time()
angular_transient_compare.backward(torch.ones_like(angular_transient))
grad = mesh.v.grad
print(' gradient')
print(time.time() - tic)
