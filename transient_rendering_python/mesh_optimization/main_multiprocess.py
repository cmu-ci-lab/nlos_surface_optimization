...
folder_name = os.getcwd() + '/progress14-%f/'%args.lr
...
for t in range(T):
    if t%2 == 0:
        if opt.w_width > 1:
           opt.w_width -= 1
           render_opt.w_width -= 1
        opt.sample_num += 50
        render_opt.sample_num += 1000    
    ...
    #tic = time.time()
    #for i in index:
    #  grad2 += worker(i, gt_transient, transient, mesh, opt)
    #print(time.time() - tic)
