...
for t in range(T):
    for i, index in enumerate(np.random.permutation(opt.lighting.shape[0])):
        ...
        scipy.io.savemat(filename, mdict={'v':np.array(mesh.v),  'transient':transient, 'l2':l2, 'origin_v': mesh_optimization.v.data.cpu().numpy(), 'grad': mesh_optimization.v.grad.data.cpu().numpy()})
