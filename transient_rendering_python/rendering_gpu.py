...
def angular_sampling(mesh, direction, lighting, sensor, lighting_normal, sensor_normal, opt):
    ...
    intersect, t, u, v = mesh_intersection_gpu.intersect_ray_mesh_batch_directions(lighting, direction, mesh, opt.epsilon)
    ...
    triangleIndexMap = Variable(torch.LongTensor(opt.sample_num).fill_(-1)).cuda()
    ...
    data = mesh_intersection_gpu.element_multiply2(1-uMap[inds] - vMap[inds], mesh.v[torch.index_select(mesh.f[:,2],0,triangleIndexMap_input),:]) + mesh_intersection_gpu.element_multiply2(uMap[inds], mesh.v[torch.index_select(mesh.f[:,0],0,triangleIndexMap_input),:]) + mesh_intersection_gpu.element_multiply2(vMap[inds], mesh.v[torch.index_select(mesh.f[:,1],0,triangleIndexMap_input),:])
    intersection_p.index_copy_(0, inds, data)


    v2[inds,:] = sensor - intersection_p[inds,:]
    d2[inds] = torch.sqrt(torch.sum(v2[inds,:]**2, 1))
    v2[inds,:] = mesh_intersection_gpu.element_divide2(v2[inds,:], d2[inds])
    intersect, t, _, _ = mesh_intersection_gpu.intersect_ray_mesh_batch_directions(sensor, -v2[inds,:], mesh, opt.epsilon)
    ...
    normalMap[inds,:] = mesh_intersection_gpu.element_divide2(fn[inds,:], fn_len[inds])
    ...
    #inds = torch.LongTensor(list(compress(range(opt.sample_num), distance_bin<=opt.max_distance_bin)))
